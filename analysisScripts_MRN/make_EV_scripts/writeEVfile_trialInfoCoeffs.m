function [regNames,  timings, info]=writeEVfile_trialInfoCoeffs(timingData, allRegModVars, evFileName, trialNum);
...
timings(2).on           =timingData.betOn;
timings(2).duration     =nanmedian  (timingData.betChoiceTime- timingData.betOn);
timings(2).name        ='bet';

timings(3).on           =timingData.forceChoiceTime;
timings(3).duration     =.5;
timings(3).name        ='forceBet';
...
% hmm... this is a bit weird... trial number sometimes starts from 2.


info.gotInfo            =find(isfinite(timingData.infoOn));
info.on                 =timingData.infoOn(info.gotInfo);
info.duration           =nanmedian(timingData.infoOff-timingData.infoOn);
...
for i = 1:ll
    ...
    timings(4).on=[ timings(4).on; trialExPush'];
end
